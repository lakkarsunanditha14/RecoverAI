...
class RecoveryActionRepository:
    ...
    def update(self, action: RecoveryAction) -> RecoveryAction:
        model = (
            self.db.query(RecoveryActionModel)
            .filter(RecoveryActionModel.action_id == action.action_id)
            .first()
        )

        if model is None:
            raise ValueError(
                f"Recovery action not found: {action.action_id}"
            )

        model.case_id = action.case_id
        model.action_type = action.action_type
        model.status = action.status
        model.proposed_at = action.proposed_at
